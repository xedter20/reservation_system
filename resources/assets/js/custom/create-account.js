...
listenClick('#submitBtn', function (event) {
    ...
    let message = ''
    let cardHolderNameRegex = /^[a-z ,.'-]+$/i
    let cvvRegex = /^[0-9]{3,3}$/
    let cardHolderName = $('#cardHolderName').val()
    let cardNumber = $('#cardNumber').val()
    let exMonth = $('#expiryMonth').val()
    let exYear = $('#expiryYear').val()
    let cvv = $('#cvv').val()
    if (cardHolderName == '') {
...
(function () {
        ...
        Range.rangeWithString = function (ranges) {
            ...
            for (j = 0, len = ranges.length; j < len; j++) {
                ...
                if (r = range.match(/^(\d+)-(\d+)$/)) {
                    for (n = k = ref = r[1], ref1 = r[2]; ref <= ref1 ? k <=
                        ref1 : k >= ref1; n = ref <= ref1 ? ++k : --k) {
                        trie.push(n)
                    }
    ...
    $.fn.validateCreditCard = function (callback, options) {
        let bind, card, card_type, card_types, get_card_type, is_valid_length,
            is_valid_luhn, j, len, normalize, ref, validate, validate_number
        card_types = [
            {
                name: 'amex',
                range: '34,37',
        ...
        get_card_type = function (number) {
            ...
            ref1 = (function () {
                ...
                for (l = 0, len1 = card_types.length; l < len1; l++) {
                    card = card_types[l]
                    if (ref1 = card.name, indexOf.call(options.accept, ref1) >=
                    0) {
                        results.push(card)
                    }
                }
        ...
        is_valid_length = function (number, card_type) {
            let ref1
            return ref1 = number.length, indexOf.call(card_type.valid_length,
                ref1) >= 0
        }
